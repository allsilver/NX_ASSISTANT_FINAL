...
| 항목 | VDI | 로컬 PC | 비고 |
|---|---|---|---|
| GPT (chatgpt.com) | ✅ 열림 | ⏳ 차주 오픈 (현재 ❌) | WebView2로 접속 |
| Gauss API (sr-cloud.com) | ❌ 막힘 | ✅ | 사내망 정책 |
| DB MCP 서버 (로컬 RAG) | ❌ 못 띄움 | ✅ | VDI는 pip/모델/Gauss 막힘 |
...
### ⚠️ GPT 분기 테스트의 딜레마 (현재)
- GPT 답변 생성 = **GPT 연결 필요** → 현재 **VDI에서만** 가능(로컬은 차주 오픈).
- GPT 분기 = **DB MCP 서버에서 프롬프트를 받아** GPT에 전달 → 그런데 **VDI엔 서버가 없음.**
- 즉 지금은 양쪽 환경 어디서도 GPT 분기 "완전" 통합 테스트가 안 됨.
- **임시 검증 방법**: VDI에서 앱 실행 → GPT 선택 → 질문. `GetGptPromptAsync`가 서버 대신 **가짜 예시 프롬프트**(가짜 컨텍스트 + 실제 질문)를 반환하도록 환경변수 플래그로 토글.
  - 플래그: **`NX_ASSISTANT_FAKE_DBPROMPT=1`** (켜면 서버 호출 없이 예시 프롬프트 사용 → GPT가 답변)
  - 검증 범위: ChatView→AskAsync(GPT 분기)→프롬프트 수신→GPT 워커 답변 까지. **서버의 실제 프롬프트 조립은 차주 로컬(GPT+서버)에서 검증.**
  - 프로덕션/실서버 테스트 땐 이 플래그를 **끄면** 원래대로 서버에서 받음.
